  ...
  return (
    <div className={`screen ${styles.wrap}`}>
      ...
      <div className={styles.intro}>
        <span className="eyebrow">{t("Voor creators")}</span>
        <h1 className={styles.h1}>{t("Welkom bij")}<br />Dinely.</h1>
      </div>

      <div className={styles.cards}>
        <Link href="/creator" className={`${styles.card} ${styles.creator}`}>
          <div className={styles.cardInner}>
            <h2>{t("Ik ben creator")}</h2>
            <p>{t("Koppel je socials en verdien met deals bij restaurants.")}</p>
            <span className={styles.go}>{t("Aan de slag")} <span className={styles.arw}>→</span></span>
          </div>
        </Link>
      </div>
    </div>
